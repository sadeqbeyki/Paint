...
{
    bool isDrawing = false; // Drawing Var
    ...
    private void panelDraw_MouseUp(object sender, MouseEventArgs e)
    {
        ...
        bool isCircle = false; // isSquare Var
        ...
        if (isDrawing == true)
        {
            ...
            int radius = Math.Min(XDiff, YDiff) / 2;
            Point center = new Point((upPoint.X + downPoint.X) / 2, (upPoint.Y + downPoint.Y) / 2);
    ...
    {
        //Draw Circle in Panel B
        graphicsPanelCanvas.DrawEllipse(penA, new Rectangle(center.X - radius, center.Y - radius, radius * 2, radius * 2)); //Graphics panel B
    }
}